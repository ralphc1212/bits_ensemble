    ...
    for idx, deno in enumerate(module.res_denos[:module.maxBWPow]):
        if s is None:
            ...
            if not weight_quantization:
                ...
                mean_split_point = mean_split_point.view(*mean_split_point.shape[:2],module.thres_mean[param_idx].shape[0],-1)
                mean_split_point = mean_split_point.mean(dim=(0,-1))

                # module.npartitions[param_idx][idx - 1] += torch.round(mean_split_point).mean(dim=(0,2)).sum().item()
                round_split = torch.cat([module.zeroD[param_idx], module.round(mean_split_point)], dim=0)
                clustering_vec = rbf_wmeans(round_split.cumsum(dim=0).unsqueeze(2), module.rbf_mu, module.rbf_sigma, module.eps)

                inner_grouped_1 = torch.einsum('abcd, bcde -> abcde', sorted_err.view(*sorted_err.shape[:2], module.thres_mean[param_idx].shape[0], -1),
                    clustering_vec.unsqueeze(2).expand(-1,-1,int(delta_sorted_err.shape[-1]/module.thres_mean[param_idx].shape[0]),-1))
...
class dense_res_bit_tree_meanvar_freeze_fine_partition_wkernel_wact_wstats_baens(nn.Module):
    def __init__(self, N=5, D1=3, D2=2, bw=3, quantize=True):
        super(dense_res_bit_tree_meanvar_freeze_fine_partition_wkernel_wact_wstats_baens, self).__init__()
        ...
        self.thres_mean = nn.ParameterList([nn.Parameter(torch.tensor([-1.3] * D2), requires_grad=True),
            nn.Parameter(torch.tensor([-1.3] * D2), requires_grad=True)])
        self.partitions = nn.ParameterList([nn.Parameter(torch.tensor([[0] * D2] * (N - 1)), requires_grad=False),
            nn.Parameter(torch.tensor([[0] * D2] * (N - 1)), requires_grad=False)])
        self.pmomentum = 0.1
...
class Conv2d_res_bit_tree_meanvar_freeze_fine_partition_wkernel_wact_wstats_baens(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True, N=5, bw=2, quantize=True, first=False):
        super(Conv2d_res_bit_tree_meanvar_freeze_fine_partition_wkernel_wact_wstats_baens, self).__init__()
        ...
        self.zeroD = nn.ParameterList([nn.Parameter(torch.zeros(1, self.D), requires_grad=False), 
            nn.Parameter(torch.zeros(1, out_channels), requires_grad=False)])
        self.thres_mean = nn.ParameterList([nn.Parameter(torch.tensor([-1.3] * out_channels), requires_grad=True),
         nn.Parameter(torch.tensor([-1.3] * out_channels), requires_grad=True)])
        self.partitions = nn.ParameterList([nn.Parameter(torch.tensor([[0] * out_channels] * (N - 1)), requires_grad=False),
         nn.Parameter(torch.tensor([[0] * out_channels] * (N - 1)), requires_grad=False)])
        self.pmomentum = 0.1
