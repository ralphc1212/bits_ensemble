...
class dense_res_bit_tree_fixed_baens(nn.Module):
    def __init__(self, N=5, D1=3, D2=2, bw=3, quantize=True, partition='2-2'):
        super(dense_res_bit_tree_fixed_baens, self).__init__()
        ...
        self.splits = [int(i) for i in partition.split('-')]
    ...
    def forward(self, x):
        ...
        for idx, deno in enumerate(self.res_denos[:self.maxBWPow]):
            if s is None:
                ...
                if 9 in self.splits:
                    grouped_err = res_errer
                elif 0 in self.splits:
                    grouped_err = res_errer.mean(dim=0, keepdim=True).expand(self.N, self.D)
                else:
                    first_piece  = res_errer[:self.splits[0]].mean(dim=0, keepdim=True).expand(self.splits[0], self.D)
                    second_pirce = res_errer[self.splits[0]:].mean(dim=0, keepdim=True).expand(self.splits[1], self.D)
                    grouped_err = torch.cat([first_piece, second_pirce])

                vals = torch.cat([vals, s * self.round(grouped_err / s).unsqueeze(0)], dim=0)
...
class Conv2d_res_bit_tree_fixed_baens(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True, N=5, bw=2, quantize=True, first=False, partition='2-2'):
        super(Conv2d_res_bit_tree_fixed_baens, self).__init__()
