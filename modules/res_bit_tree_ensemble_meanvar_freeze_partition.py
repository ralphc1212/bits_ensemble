...
class dense_res_bit_tree_meanvar_freeze_partition_baens(nn.Module):
    def __init__(self, N=5, D1=3, D2=2, bw=3, quantize=True):
        super(dense_res_bit_tree_meanvar_freeze_partition_baens, self).__init__()
        ...
        self.thres_mean = nn.Parameter(torch.tensor([-1.3]), requires_grad=True)
        ...
        for idx, deno in enumerate(self.res_denos[:self.maxBWPow]):
            if s is None:
                ...
                if self.update_partition:
                    ...
                    self.split_point = split_point.detach()
                    ...
                    transfered_errors = torch.einsum('ab, cba -> cb', res_errer, self.transfer_matrix.detach())
                    self.npartitions[idx - 1] += torch.round(self.split_point.detach()).sum().item()

                    buf = 0.
                    local_cnt = 0
                    t_l = []

                    for iidx, err in enumerate(transfered_errors):
                        buf += err
                        local_cnt += 1
                        if iidx == len(transfered_errors) - 1:
                            t_l.append((buf / local_cnt).unsqueeze(0).expand(local_cnt, -1))
                        elif torch.round(self.split_point[iidx]):
                            t_l.append((buf / local_cnt).unsqueeze(0).expand(local_cnt, -1))
                            buf = 0.
                            local_cnt = 0

                    grouped = torch.einsum('cb, cba -> ab', torch.cat(t_l, dim=0), self.transfer_matrix.detach())
                    quantized = s * self.round(grouped / s)
...
class Conv2d_res_bit_tree_meanvar_freeze_partition_baens(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True, N=5, bw=2, quantize=True, first=False):
        super(Conv2d_res_bit_tree_meanvar_freeze_partition_baens, self).__init__()
