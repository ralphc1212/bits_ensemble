...
class dense_res_bit_tree_baens(nn.Module):
    def __init__(self, N=5, D1=3, D2=2, bw=3, quantize=True):
        super(dense_res_bit_tree_baens, self).__init__()
        ...
        self.thres = nn.Parameter(torch.tensor([0.5]), requires_grad=True)
    ...
    def forward(self, x):
        ...
        for idx, deno in enumerate(self.res_denos[:self.maxBWPow]):
            if s is None:
                ...
                # use the the aggregated error at the beginning
                ...
                split_point = df_lt(delta_sorted_err, self.thres, self.temperature)
...
class Conv2d_res_bit_tree_baens(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True, N=5, bw=2, quantize=True, first=False):
        super(Conv2d_res_bit_tree_baens, self).__init__()
