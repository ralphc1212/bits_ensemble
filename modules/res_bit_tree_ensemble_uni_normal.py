...
class dense_res_bit_tree_uni_normal_baens(nn.Module):
    def __init__(self, N=5, D1=3, D2=2, bw=3, quantize=True):
        super(dense_res_bit_tree_uni_normal_baens, self).__init__()
        ...
        # self.thres_mean = nn.Parameter(torch.tensor([-1.2]), requires_grad=True)
        # self.thres_var  = nn.Parameter(torch.tensor([-3.2]), requires_grad=True)

        self.thres_mu = nn.Parameter(torch.tensor([0.1]) * torch.ones(self.U.shape[0]-1, 1), requires_grad=True)
        self.thres_sigma  = nn.Parameter(torch.ones(self.U.shape[0]-1, 1), requires_grad=True)
    ...
    def forward(self, x):
        ...
        for idx, deno in enumerate(self.res_denos[:self.maxBWPow]):
            if s is None:
                ...
                samples = torch.distributions.normal.Normal(self.thres_mu, torch.sigmoid(self.thres_sigma)).rsample()
                splits = df_lt(delta_sorted_err, torch.sigmoid(samples), 0.01)
                ...
                vals = torch.cat([vals, s * self.round(torch.cat(t_l, dim=0) / s).unsqueeze(0)], dim=0)
...
class Conv2d_res_bit_tree_uni_normal_baens(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True, N=5, bw=2, quantize=True, first=False):
        super(Conv2d_res_bit_tree_uni_normal_baens, self).__init__()
