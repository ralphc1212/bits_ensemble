...
from models import VGG_res_bit_tree_meanvar_freeze_fine_partition_wkernel_kv_gn_ensemble, ECELoss
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        # loss = mixup_criterion(criterion, outputs, targets_a, targets_b, lam, True)
...
def tune(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        # inputs = torch.stack([data_transform(inputs[int(i*batch_size) : int((i+1)*batch_size)]) for i in range(nmembers)])
    ...
    print('nPartitions: {}'.format(np.mean(np.array(partis), axis=0)))
    print('Train loss: %.3f' % np.mean(train_loss))
    print('Train accuracy: %.3f%%' % (correct * 100.0/total))
...
special_suffix = '300epoch.sgd.periodsplit'
MODEL = 'data-cifar10.vgg11.kv-gn-baens'
...
for setup in setups:
    ...
    model = VGG_res_bit_tree_meanvar_freeze_fine_partition_wkernel_kv_gn_ensemble(10, True, '11', *setup, bias=False)
    model.cuda()
    ...
    scheduler2 = torch.optim.lr_scheduler.MultiStepLR(optim_v, milestones=[60, 100, 140, 180, 220, 260], gamma=0.1)
